...
const localClient = new RumpusCE(process.env.RUMPUS_DELEGATION_KEY, "local");

const devClient = new RumpusCE(process.env.RUMPUS_DELEGATION_KEY, "dev", {
  ...
  describe("General", async function() {
    it("can fetch the server version", async function() {
      ...
      for (const field of versionFields) {
        expect(
          version[field as VersionedItem],
  ...
  describe("Levelhead", async function() {
    ...
    describe("Bookmarks", async function() {
      it("can add and remove bookmarks", async function() {
        ...
        const topLevels = await rce.levelhead.levels.search({
          sort: "-QAScore",
          minPlayers: 20,
          limit: 5
        });
        for (const level of topLevels) {
          ...
          let bookmarked = await rce.levelhead.bookmarks.search({
            levelIds: level.levelId
          });
          expect(bookmarked.length, "bookmark search should work").to.equal(1);
          expect(bookmarked[0], "bookmark must be the one we added").to.equal(
            level.levelId
          );

          // Remove it, and make sure it's GONE.
          await rce.levelhead.bookmarks.remove(level.levelId);
          bookmarked = await rce.levelhead.bookmarks.search({
            levelIds: level.levelId
          });
          expect(
            bookmarked.length,
            "bookmark search should return nothing"
          ).to.equal(0);
        }
      });
    });
    describe("Levels", async function() {
      ...
      it("can fetch level likes", async function() {
        const likedLevel = (
          await rce.levelhead.levels.search({ limit: 1, sort: "Likes" })
        )[0];
      ...
      it("can page level favorites", async function() {
        ...
        while (minFavorites < maxFavorites) {
          ...
          const favoritedLevel = (
            await rce.levelhead.levels.search({
              limit: 1,
              sort: "-Favorites",
              minFavorites,
              maxFavorites
          ...
          const allFavorites = await rce.levelhead.levels.getFavorites(
            favoritedLevel.levelId,
            { limit: 99 }
          );
        ...
        let page = await rce.levelhead.levels.getFavorites(levelId, {
          limit: 1
        });
    ...
    describe("Players", async function() {
      ...
      it("can (un)follow a player", async function() {
        ...
        const isFollowingDev = async () => {
          const follows = await rce.levelhead.players.getFollowing(userId, {
            userIds: dev
          });
        ...
        if (await isFollowingDev()) {
          ...
          expect(await isFollowingDev(), "should no longer be following").to.be
            .false;
          await rce.levelhead.players.follow(dev);
          expect(await isFollowingDev(), "should be following again").to.be
            .true;
        } else {
          await rce.levelhead.players.follow(dev);
          expect(await isFollowingDev(), "should now be following").to.be.true;
          await rce.levelhead.players.unfollow(dev);
          expect(await isFollowingDev(), "should not be following again").to.be
            .false;
        }
      });
