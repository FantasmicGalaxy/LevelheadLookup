...
export function attachAvatarUrlToItem<item extends AvatarItemBase>(item: item) {
  item.avatarUrl = (pixels?: number, imgType: ImageType = "png") =>
    avatarUrl(item.avatarId, pixels, imgType);
  return item;
}

/** Add avatar-URL-generating `avatarUrl()` methods to an objects that have an `avatarId` field. */
export function attachAvatarUrlToArrayItems<item extends AvatarItemBase>(
  items: item[]
) {
...
export function cleanQuery(query?: {
  [param: string]: string | boolean | string[] | number | null | undefined;
}) {
